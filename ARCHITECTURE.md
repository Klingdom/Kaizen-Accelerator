...
| `TodayPageViewed` | `{ userId: string, fromRoute: string\|null, viewedAt: string, layoutVersion?: string }` | MetricsService (future — latency baseline anchor; computes `comprehension_seconds = first_meaningful_action.timestamp - TodayPageViewed.timestamp`). Emitted once per route-entry from app.js route-change handler — NOT on every render. Timestamp via `services.clock.now()`. Added Iteration 21 (C-AN-1). `layoutVersion` optional field added in C-UX-COL column refactor: `'v2'` for post-refactor sessions (outputArtifact column visible, state-label removed), absent for pre-refactor sessions. Enables cohort split at query time without resetting the 14-day baseline. |
| `EditDrawerOpened` | `{ userId: string, compositionId: string, openedAt: string }` | MetricsService (future — splits accept-path vs edit-path latency: `edit_path_seconds = ActivityStarted.startedAt - EditDrawerOpened.openedAt`). Emitted once per EDIT action handler invocation in app.js. Timestamp via `services.clock.now()`. Added Iteration 21 (C-AN-1). |
| `RowOutputClicked` | `{ userId: string, activityId: string, catalogEntryId: string\|null, clickedAt: string }` | MetricsService (future — proactive artifact engagement rate: did the user open the artifact dialog before close, not just at close time?). Emitted by app.js `OPEN_OUTPUT_ARTIFACT` handler when user clicks `.sa-artifact` on a Today page row. Timestamp via `services.clock.now()`. Added in C-UX-COL column refactor (Q2 decision: `.sa-artifact` is clickable). |
| `CISkipConfirmed` | `{ userId: string, activityId: string, catalogEntryId: string, reasonCode: string, confirmedAt: string }` | MetricsService (future — CI sacredness telemetry: detect whether CI is dropped at skip-time vs compose-time, to guide escalation from option (c) UI confirm-on-skip to option (a) isSacred flag). Emitted by app.js `SUBMIT_SKIP_MODAL` handler when `isCIActivity === true` in the payload. `confirmedAt` via `services.clock.now()`. No immediate consumer. Added Iter 38 Phase B (SW-Q10 decision: confirm-on-skip + telemetry, ARCHITECTURE_DELTA_TODAY_SIMPLIFY.md §6.3 option c). |
