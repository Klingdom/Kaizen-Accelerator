...
| `TodayPageViewed` | `{ userId: string, fromRoute: string\|null, viewedAt: string }` | MetricsService (future — latency baseline anchor; computes `comprehension_seconds = first_meaningful_action.timestamp - TodayPageViewed.timestamp`). Emitted once per route-entry from app.js route-change handler — NOT on every render. Timestamp via `services.clock.now()`. Added Iteration 21 (C-AN-1) to start the 14-day pre-redesign baseline window. |
