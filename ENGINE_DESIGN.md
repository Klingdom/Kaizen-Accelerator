...
| AD_HOC project exceeds declared `targetCloseDate` while still in `{DRAFT, ACTIVE, IN_REMEASUREMENT}` | `ProjectPaceWarning` with `kind='AD_HOC_OVERRUN'` | UI (inline microcopy on `KaizenCard` — "AD_HOC project past target close date; decide close, extend, or abandon"), `MetricsService` |
| User files a scope change on Accelerator, DMAIC, Kaizen 90, or AD_HOC | `ScopeChangeRequested` | `KaizenService.appendScopeChange()` (appends to `Kaizen.scopeChanges[]`), UI, `MetricsService` |
| PDCA experiment reaches 10th tick without `PdcaExperimentClosed` | `PdcaTickCommitted` | Reflection agent (`AI_AGENTS.md §2.4` R5) — surfaces mandatory-review banner on `/today`; not a hard block |
