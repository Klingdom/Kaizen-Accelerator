...
| 7 | C-PM-CAL-P2 | Today calendar Phase 2: drag-to-move + drag-to-resize | improvement | 12 | 6 | **OPEN — Phil-blocked** | Pointer-events drag implementation; 15-min snap; bottom-edge resize handle. Reuses existing EDIT_CHANGE_START_TIME + EDIT_CHANGE_DURATION dispatch (0 §6.5 hits expected). HIGH risk per QA (zero pointer-event coverage; new state machines: INFEASIBLE guard + IN_PROGRESS protection + conflict detection). **Blocks on SW-Q-CAL-01 (drag commit semantics) + SW-Q-CAL-03 (conflict policy)**. Critical gate per Iter 28 hotfix lesson: drag must require `composition.state !== 'PROPOSED'` AND block when `activity.state === 'IN_PROGRESS'`. |
...
| 6 | C-FE-1 | BROWSER_CATALOG missing `recovery_lunch` entry | fix | 8 | 1 (FE follow-up from Iter 28) | OPEN | Iter 26 added `recovery_lunch` to seed catalog (60→61) but BROWSER_CATALOG (client-side bundled catalog) was not synced. Lunch row may render without Expected Output column populated. Trivial: regenerate browser catalog from full catalog. ~30 min. |
