...
| 4 | C-UX-FUTURISTIC-P2 | Luminous Constraint Phase 2: typography swap + depth + block evolution | improvement | 14 | 7 | **OPEN — ready post-deploy** | Switch DM trio → Instrument Serif + Geist + Geist Mono (Google Fonts). 3-stop inner-top-light gradients on block fills. Hour-rail typography refinement. Now-line glow refinement. ~6-8 hr. MEDIUM risk per QA (z-index risk if depth layers inserted into `.cycle-timeline`). 0 §6.5 hits expected. Depends on Phase 1 (Iter 39 — DONE). |
