...
**Status: v0.2 — all 31 procedures authored to Master Black Belt / BAM standard. Phil review requested; content treated as locked for E13-T1 unless Phil redlines.**

The four "no X without Y" engine invariants from §1.1 are enforced by specific procedure steps in §3:
- **No improvement without baseline** → Phase 1 procedures `30d_1_1` (data capture), `30d_1_3` (activity metrics), and `30d_1_6` (lock `BaselineMetric`) form the chain. Advancement to PHASE_2 fails without them.
- **No solution without root cause** → `30d_2_3` step (a) + (b) produce 5 Whys + Fishbone output; `30d_2_4` Future-State Design `dependsOn` this.
- **No implementation without ownership** → `30d_3_1` steps (a)–(d) require a named owner and a non-null due date per backlog item before `Kaizen.actions[]` is populated.
- **No success without validated ROI** → `30d_4_3` writes both `implementationCostDollars` AND `annualBenefitsDollars`; `30d_4_4` Validate Results `dependsOn` 4.3.
...
|---|---|---|---|---|---|---|---|---|---|---|
| `30d_0_1_identify_candidate_process` | Identify Candidate Process | COMMUNICATION | 60 | EVENT_DRIVEN | Engagement kickoff | Leadership input, pain-point list, KPI gap report | `{ name: "Candidate process list", schema: "TEXT", required: true }` | Facilitator + Leader(s) | a. Interview 2–3 leaders using a 30-minute Voice-of-Leader template (top 3 pain points, top 3 KPI gaps, escalation history). b. Pull quantitative KPI-gap rows from the last quarter scorecard; tag each with affected process. c. Draft candidate process list (3–7 candidates) with one-line pain and one-line measurable-gap per candidate. d. Short-list 1–2 with Leader using an Impact × Feasibility 2×2 scored out of 5 on each axis. | [] |
...
|---|---|---|---|---|---|---|---|---|---|---|
| `30d_1_1_collect_workflow_data` | Collect Workflow Data | PROJECT | 240 | EVENT_DRIVEN | Charter signed | Systems access list, metric definitions from `30d_0_4`, stopwatch / observation log (MVP), Ledgerium AI recording kit (Next-phase integration, not MVP) | `{ name: "Raw workflow data", schema: "DOCUMENT", required: true }` (link to dataset, time-study log, or recording) | Facilitator + SMEs | a. Choose capture method: **Option A — Existing data:** query system logs / export workflow reports / sample tickets; pull minimum 30 completed process instances covering at least 2 weeks. **Option B — Direct observation (MVP default when no system logs exist):** shadow 3–5 end-to-end runs using a manual time-study method — stopwatch + observation log capturing step-by-step start / stop / wait / system-of-record / handoff. Ledgerium AI automated recording is a Next-phase integration and is **not** required for MVP. b. For the chosen option, execute collection against the metric definitions from `30d_0_4`. c. Log any data-quality flags (missing timestamps, ambiguous step boundaries, interrupted runs) — do not discard; annotate. d. Store the raw dataset or observation log in the Kaizen workspace with an access link and dataset-version tag. | [`30d_0_6_approve_charter`] |
...
| Id | Name | Bucket | Default min | Cadence | Trigger | Inputs | Output artifact | Participants | Procedure | dependsOn |
|---|---|---|---|---|---|---|---|---|---|---|
| `30d_4_1_rebaseline` | Rebaseline Workflow | PROJECT | 180 | EVENT_DRIVEN | Phase 4 entered | Executed changes log, systems access, stopwatch / observation log (MVP), Ledgerium AI (Next-phase), locked `BaselineMetric` from `30d_1_6` | `{ name: "Post-implementation dataset", schema: "DOCUMENT", required: true }` — becomes the `Remeasurement` row on `Kaizen` | Facilitator + SMEs | a. Repeat the exact capture method used in `30d_1_1_collect_workflow_data`. **Option A — Existing data:** re-query the same system logs / workflow reports; pull 30 post-implementation process instances. **Option B — Direct observation (MVP default):** shadow 3–5 post-implementation end-to-end runs using the same stopwatch + observation-log template used at baseline. Ledgerium AI re-recording remains Next-phase only. b. Use the same metric definitions, operational definitions, and sampling frame as baseline to prevent apples-to-oranges comparison; if any definition changed, flag and document. c. Log data-quality flags identically to baseline. d. Store the post-implementation dataset in the Kaizen workspace and link it as the `Remeasurement` artifact (same `metricDefinitionId` as `BaselineMetric`). | [] |
