...
_Last updated: 2026-04-29 after Iteration 18 (Operating-Model Update — META_REVIEW §6.1–§6.6 adopted). All 6 meta-review recommendations applied to `.claude/agents/coordinator.md` and `IMPROVEMENT_BACKLOG.md`. New operating model: bundling = "ONE coherent shipment per loop" with 4 conjunctive conditions; Pre-Flight Reconnaissance mandatory; Define-pass required for Effort ≥ M / defects / score ≥ 13; ConvergenceBonus added to scoring formula with score-13 gate; composer/engine integrity boundary as written rule; Lens count + Lenses fields required on every candidate. Iteration 19 recommendation queued: C-UX-6 + C-UX-8 + C-AN-1 bundle (would be the first iteration explicitly run under the new §6.1 bundling rule)._
