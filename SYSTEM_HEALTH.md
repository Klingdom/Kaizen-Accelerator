...
_Last updated: 2026-04-30 after Iteration 21 ("Baseline + Safe Fix" — C-AN-1 + C-UX-2 + C-QA-V2-1). 3-item bundle dispatched and shipped together: (a) C-AN-1 instrumentation — `TodayPageViewed` + `EditDrawerOpened` events added to `js/events/events.js` (36→38) and `ARCHITECTURE.md §6.1`; emit sites at `js/app.js:1079` (Edit drawer) and `js/app.js:2493` (Today route, guarded by `_prevRoute` sentinel); (b) C-UX-2 BucketStrip blackout fix — `app.css` opacity selector narrowed from `.cycle-card:not(.cycle-editing)` to `.cycle-activities` + `.triad` only, closing an 8-iteration-old defect; (c) C-QA-V2-1 CCC proxy — `tests/ui/pages/Today.ccc.test.js` (245 lines, 9 tests) automated guard for ≤12 regions and ≤25 words/region. Suite 2,843 → 2,866 tests (+23), runtime 3.15s (10% headroom recovered). All 15 ACs PASS. §6.5 boundary preserved (no composer/engine/events/types touched)._

_Iter 22+ queue (held until 14-day baseline closes): C-UX-V2-1 auto-collapse RhythmExplainer (15), C-UX-V2-2 single Commit + keyboard (14), C-UX-6 focus traps (16). V2-2 scope decision pending: (a) label-fix-only or (b) unified drawer redesign. Default if no decision: (a)._
