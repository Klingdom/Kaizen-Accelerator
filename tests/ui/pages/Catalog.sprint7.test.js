...
describe('Catalog — bucket count accuracy against full 85-entry seed (Phase 1: +4 convergent Tier 1; Phase 2: +6 lens-unique Tier 1; Phase 3: +10 Tier 2; Phase 4: +4 Tier 3)', () => {
  // Defensive test — uses the actual seed pipeline to verify no silent
  // bucket mapping drift per Sprint 7 hard rule.
  test('seeded catalog has 85 entries total (Iter 26: +1 recovery_lunch; Phase 1: +4; Phase 2: +6; Phase 3: +10; Phase 4: +4)', async () => {
    const { buildCatalog } = await import('../../../js/catalog/seed/index.js');
    const cat = buildCatalog();
    assert.equal(cat.length, 85);
  });

  test('bucket counts for PROJECT/COMMUNICATION/CI sum to 84 (recovery_lunch excluded — bucket null)', async () => {
    const { buildCatalog } = await import('../../../js/catalog/seed/index.js');
    const cat = buildCatalog();
    ...
    assert.equal(counts.PROJECT + counts.COMMUNICATION + counts.CI, 84);
