...
describe('Catalog — bucket count accuracy against full 60-entry seed', () => {
  // Defensive test — uses the actual seed pipeline to verify no silent
  // bucket mapping drift per Sprint 7 hard rule.
  test('seeded catalog has 60 entries total', async () => {
    const { buildCatalog } = await import('../../../js/catalog/seed/index.js');
    const cat = buildCatalog();
    assert.equal(cat.length, 60);
  });

  test('bucket counts sum to 60 and split PROJECT / COMMUNICATION / CI', async () => {
    const { buildCatalog } = await import('../../../js/catalog/seed/index.js');
    const cat = buildCatalog();
