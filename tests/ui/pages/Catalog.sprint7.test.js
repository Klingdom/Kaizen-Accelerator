...
describe('Catalog — bucket count accuracy against full 61-entry seed (Iter 26)', () => {
  // Defensive test — uses the actual seed pipeline to verify no silent
  // bucket mapping drift per Sprint 7 hard rule.
  test('seeded catalog has 61 entries total (Iter 26: +1 recovery_lunch)', async () => {
    const { buildCatalog } = await import('../../../js/catalog/seed/index.js');
    const cat = buildCatalog();
    assert.equal(cat.length, 61);
  });

  test('bucket counts for PROJECT/COMMUNICATION/CI sum to 60 (recovery_lunch excluded — bucket null)', async () => {
    const { buildCatalog } = await import('../../../js/catalog/seed/index.js');
    const cat = buildCatalog();
    ...
    assert.equal(counts.PROJECT + counts.COMMUNICATION + counts.CI, 60);
